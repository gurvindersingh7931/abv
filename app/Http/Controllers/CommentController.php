...
//    	return $comment;
        $project = $issue->task->project->name;
        //echo "<pre>".$project; die;
        $employees = User::where('privilage', 2)->get();

        $issue_data = Issue::where('id', $comment->issue_id)->first();
//        auth()->user()->name;
        foreach ($employees as $employee){
            $employee->notify(new CommentAdded($project, $issue_data->title, auth()->user()->name, $issue_data->id, $issue_data->task_id));
        }
    	return back();
    }
